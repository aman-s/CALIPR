...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[Vint]=IDW_3D_mitRes_DKU(Res, xc,yc, zc, vc,x,y, z, e,r1,r2)
%%%%%%%%%%%%%%%%%
%%% INPUTS
...
%   $Updated 2012/02/24 $ 
%
%  Modified by  Dushyant Kumar
% i) Generalized for 3D case
% ii) Added resolution "Res"
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Vint=zeros(length(x), length(y), length(z));
xc=reshape(xc,1,length(xc));
yc=reshape(yc,1,length(yc));
zc=reshape(zc,1,length(zc));
vc=reshape(vc,1,length(vc));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Rx = Res(1);   Ry = Res(2);   Rz = Res(3);
...
if  strcmp(r1,'fr')
    ...
    for i=1:length(x)
        for j=1:length(y)
            for k = 1:length(z)
                D=[]; V=[]; wV =[]; vcc=[];
                %D= sqrt((x(i)-xc).^2 +(y(j)-yc).^2 +(z(i)-zc).^2 );
                D= sqrt((Rx*(x(i)-xc)).^2 +(Ry*(y(j)-yc)).^2 +(Rz*(z(k)-zc).^2));
                if min(D)==0
