function DK_nii_NNLS(T2Data, matmask, Resol, TE, TR, T2Range, spwin, nT2, nCores, nThreads, FAE_in_fin_step, RegType, nSpatIter, Size_DSW, Overlap_DSW, SpatRegOpt, SpatRegSetAlpha, SpatReg_Init_FA, SpatReg_Init_T2)
    ...
    nii = load_untouch_nii(T2Data);
    nii_mask = load_untouch_nii(matmask);

    nEchoes = size(nii.img,4)
    ...
    % nii_mask.hdr.dime.dim(2)     = size(nii_mask.img, 1);
    % nii_mask.hdr.dime.dim(3)     = size(nii_mask.img, 2);
    % nii_mask.hdr.dime.dim(4)     = size(nii_mask.img, 3);    
    % nii_mask.hdr.dime.dim(5)     = size(nii_mask.img, 4);   
    nii_mask.hdr.hist.descrip = NiiStr;  % Extra string containing echo info

    disp('nifti details commented')

    save_untouch_nii(nii, [ Foldername '/' save_prefix '.nii' ]);
    save_untouch_nii(nii_mask, [ Foldername '/' save_prefix '_mask.nii' ]);
   ...
   % '2D_3D_Mixed' was later on added
    ...
    for i = 1:length(DataFiles_wt_Dir_Cell)
        ...
        if(~isempty(Filename_wt_Folder))
            ...
            MatPriorityTag = 'high priority';  % 'normal', 'low', 'below normal', 'above normal', 'high priority', 'real time'
